...
- Add `From<alloc::vec::Vec<u8>>` implementation for `Bytes`
- Add `TryFrom<alloc::vec::Vec<u8>>` and `TryFrom<alloc::boxed::Box<[u8]>>` implementations for `MqttBinary`
- Add `TryFrom<alloc::string::String>>`, `TryFrom<alloc::vec::Vec<u8>>`, `TryFrom<alloc::boxed::Box<str>>` and `TryFrom<alloc::boxed::Box<[u8]>>` implementations for `MqttString`
...
- Add const generic parameter `MAX_INCOMING_TOPIC_ALIASES` indicating the number of topic aliases that are supported in inbound direction
- Add const generic parameter `MAX_OUTGOING_TOPIC_ALIASES` indicating the number of topic aliases that are supported in outbound direction
...
- Use `NonZero<T>` optimisations in `KeepAlive`, `MaximumPacketSize` and `ReceiveMaximum`
- Replace all packet identifier occurrences as `u16` with `PacketIdentifier` newtype around `NonZero<u16>`
...
- Add `log-level-*` and `log-verbose` feature flags for more granular logging
